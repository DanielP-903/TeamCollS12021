Shader "Water/Gerstner Ortho"
{
	...
	SubShader
	{
		...
		uniform float _FoamDepth, _FoamPower;
		...
		void surf( Input i , inout SurfaceOutputStandard o )
		{
			...
			float4 ase_screenPosNorm = ase_screenPos / ase_screenPos.w;
			ase_screenPosNorm.z = ( UNITY_NEAR_CLIP_VALUE >= 0 ) ? ase_screenPosNorm.z : ase_screenPosNorm.z * 0.5 + 0.5;
			float eyeDepth69 = SAMPLE_DEPTH_TEXTURE( _CameraDepthTexture, ase_screenPosNorm.xy );
			float lerpResult7 = lerp( _ProjectionParams.y , _ProjectionParams.z , ( _ProjectionParams.x > 0.0 ? eyeDepth69 : ( 1.0 - eyeDepth69 ) ));
			float lerpResult16 = lerp( _ProjectionParams.y , _ProjectionParams.z , (0.0 + (ase_screenPos.z - 1.0) * (1.0 - 0.0) / (0.0 - 1.0)));
			...
			float temp_output_94_0 = saturate( ( pow( ( ( lerpResult7 - lerpResult16 ) / _Depth ) , _Falloff ) + fresnelNode93 ) );
			...
			float distanceDepth96 = saturate( abs( ( lerpResult7 -lerpResult16 ) / ( _DepthFadeDistance ) ) );
			o.Alpha = ( temp_output_94_0 * distanceDepth96 );
			o.Normal = o.Normal + 0.00001 * i.screenPos * i.worldPos;

			float foam =  pow(saturate(1 - (( lerpResult7 - lerpResult16 ) / _FoamDepth)), _FoamPower);
